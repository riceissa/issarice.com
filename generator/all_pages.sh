...
cat << EOF
This is a list of all pages on this site, ordered by the date of last
modification. Note that even insubstantial edits are taken into account when
calculating the last modification date, so the ordering differs from the
ordering obtained when sorting by the date when the last substantial edit was
made on each page. For the latter, see [List of pages sorted by date of last
substantive revision](_all_date).
